...
#define TEMPERATURE_ENDPOINT         1
#define COORDINATOR_NODE_ID          0x0000
#define TEMPERATURE_REPORT_PERIOD_MS 5000
#define AUTO_JOIN_DELAY_MS           3000
#define AUTO_JOIN_RETRY_MS           30000

static bool commissioning = false;
static uint8_t lastButton;
static int16_t simulated_temperature_centi_c = 2500;
...
static sl_zigbee_af_event_t temperature_report_event;
static sl_zigbee_af_event_t auto_join_event;
...
static void auto_join_event_handler(sl_zigbee_af_event_t *event)
{
  sl_status_t status;

  if (sl_zigbee_af_network_state() == SL_ZIGBEE_JOINED_NETWORK) {
    status = sl_zigbee_find_and_rejoin_network(
      true,
      SL_ZIGBEE_ALL_802_15_4_CHANNELS_MASK,
      SL_ZIGBEE_REJOIN_DUE_TO_APP_EVENT_1,
      SL_ZIGBEE_DEVICE_TYPE_UNCHANGED);
    sl_zigbee_app_debug_println("Auto rejoin start: 0x%02X", status);
    return;
  }

  if (!commissioning) {
    status = sl_zigbee_af_network_steering_start();
    commissioning = (status == SL_STATUS_OK);
    sl_zigbee_app_debug_println("Auto join start: 0x%02X", status);
  }

  sl_zigbee_af_event_set_delay_ms(event, AUTO_JOIN_RETRY_MS);
}
static int16_t read_temperature_centi_celsius(void)
{
  // Test source: 25.00 C -> 26.00 C. Replace this function with the real
  // bedside sensor driver; the Zigbee payload remains a signed 16-bit value.
  int16_t value = simulated_temperature_centi_c;
  simulated_temperature_centi_c += 10;
  if (simulated_temperature_centi_c > 2600) {
    simulated_temperature_centi_c = 2500;
  }
  return value;
}

static void temperature_report_event_handler(sl_zigbee_af_event_t *event)
{
  if (sl_zigbee_af_network_state() == SL_ZIGBEE_JOINED_NETWORK) {
    int16_t temperature = read_temperature_centi_celsius();
    uint8_t report[8];
    uint8_t report_length = 0;
    sl_zigbee_af_status_t zcl_status;

    zcl_status = sl_zigbee_af_write_server_attribute(
    ...
    if (zcl_status == SL_ZIGBEE_ZCL_STATUS_SUCCESS) {
      zcl_status = sl_zigbee_af_append_attribute_report_fields(
        TEMPERATURE_ENDPOINT,
        ZCL_TEMP_MEASUREMENT_CLUSTER_ID,
        ZCL_TEMP_MEASURED_VALUE_ATTRIBUTE_ID,
        CLUSTER_MASK_SERVER,
    ...
    if (zcl_status == SL_ZIGBEE_ZCL_STATUS_SUCCESS) {
      sl_zigbee_af_fill_command_global_server_to_client_report_attributes(
        ...
        report,
        report_length);
      sl_zigbee_af_set_command_endpoints(TEMPERATURE_ENDPOINT, 1);
      sl_status_t send_status = sl_zigbee_af_send_command_unicast(
        SL_ZIGBEE_OUTGOING_DIRECT,
        COORDINATOR_NODE_ID);
      sl_zigbee_app_debug_println("Temperature: %d.%02d C, send: 0x%02X",
                                 temperature / 100,
                                 temperature % 100,
                                 send_status);
    } else {
      sl_zigbee_app_debug_println("Temperature attribute error: 0x%02X", zcl_status);
    }
  }

  sl_zigbee_af_event_set_delay_ms(event, TEMPERATURE_REPORT_PERIOD_MS);
}
...
void sl_zigbee_af_main_init_cb(void)
{
  ...
  sl_zigbee_af_event_init(&temperature_report_event, temperature_report_event_handler);
  sl_zigbee_af_event_init(&auto_join_event, auto_join_event_handler);
  sl_zigbee_af_event_set_delay_ms(&auto_join_event, AUTO_JOIN_DELAY_MS);
  sl_zigbee_af_event_set_delay_ms(&temperature_report_event,
                                  TEMPERATURE_REPORT_PERIOD_MS);
}
...
void sl_zigbee_af_stack_status_cb(sl_status_t status)
{
  if (status == SL_STATUS_NETWORK_DOWN) {
    ...
    led_turn_on(COMMISSIONING_STATUS_LED);
    sl_zigbee_af_event_set_active(&temperature_report_event);
  }
}
