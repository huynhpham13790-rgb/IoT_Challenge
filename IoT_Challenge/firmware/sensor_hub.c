...
/* Nháy cả 3 LED cùng lúc - dấu hiệu nhận biết firmware version bằng mắt.
 * Blocking, chỉ gọi từ app_init(). Xem chú thích trong sensor_hub.h. */
void sh_alert_blink_all(uint8_t times, uint16_t on_ms, uint16_t off_ms)
{
  for (uint8_t i = 0; i < times; i++) {
    alert_pin_write(LED_GREEN_PORT,  LED_GREEN_PIN,  true);
    alert_pin_write(LED_YELLOW_PORT, LED_YELLOW_PIN, true);
    alert_pin_write(LED_RED_PORT,    LED_RED_PIN,    true);
    sl_sleeptimer_delay_millisecond(on_ms);

    alert_pin_write(LED_GREEN_PORT,  LED_GREEN_PIN,  false);
    alert_pin_write(LED_YELLOW_PORT, LED_YELLOW_PIN, false);
    alert_pin_write(LED_RED_PORT,    LED_RED_PIN,    false);
    sl_sleeptimer_delay_millisecond(off_ms);
  }

  /* Trả về đúng trạng thái nghỉ mà alert_init() đã đặt. alert_level vẫn là
   * ALERT_GREEN nên sh_alert_set_level() sau này không bị lệch trạng thái. */
  alert_pin_write(LED_GREEN_PORT,  LED_GREEN_PIN,  true);
  alert_pin_write(LED_YELLOW_PORT, LED_YELLOW_PIN, false);
  alert_pin_write(LED_RED_PORT,    LED_RED_PIN,    false);
}
